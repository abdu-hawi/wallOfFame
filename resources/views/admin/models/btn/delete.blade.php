...
<button type="button" class="btn btn-outline-danger btn-sm" id="delete_{!! $id !!}" data-toggle="modal" data-target="#models_delete_modal_{!! $id !!}">
    <i class="fa fa-trash"></i>
</button>

<!-- The Modal -->
<div class="modal fade" id="models_delete_modal_{!! $id !!}">
    <div class="modal-dialog">
        <div class="modal-content">

            <!-- Modal Header -->
            <div class="modal-header bg-danger">
                <h4 class="modal-title">{!! trans('admin.Delete Model') !!}</h4>
        ...
        <!-- Modal body -->
            <div class="modal-body">
                <h5>{!! trans('admin.Are you sure you want delete model',['s'=>'( ','name'=>$nick_name,'q'=>' )?']) !!}</h5>
            </div>

            <!-- Modal footer -->
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-dark" data-dismiss="modal">{!! trans('admin.Close') !!}</button>
                <button type="submit" class="btn btn-outline-danger delete-model-{!! $id !!}" data-dismiss="modal">{!! trans('admin.Agree') !!}</button>
            </div>
...
{!! Form::open(['url' => aurl('models/'.$id),'id'=>'form_delete_model_'.$id,'method'=>'delete']) !!}
...
<script>
    $(document).on('click','.delete-model-{!! $id !!}',function(){
        $('#form_delete_model_{!! $id !!}').submit();
    });

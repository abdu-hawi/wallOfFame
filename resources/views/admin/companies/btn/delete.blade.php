...
<button type="button" class="btn btn-outline-danger btn-sm" id="delete_{!! $id !!}" data-toggle="modal" data-target="#companies_delete_modal_{!! $id !!}">
    <i class="fa fa-trash"></i>
</button>

<!-- The Modal -->
<div class="modal fade" id="companies_delete_modal_{!! $id !!}">
    <div class="modal-dialog">
        <div class="modal-content">

            <!-- Modal Header -->
            <div class="modal-header bg-danger">
                <h4 class="modal-title">{!! trans('admin.Delete Company') !!}</h4>
        ...
        <!-- Modal body -->
            <div class="modal-body">
                <h5>{!! trans('admin.Are you sure you want delete company',['s'=>'( ','name'=>$gm_name,'company'=>$company_name,'q'=>' )?']) !!}</h5>
            </div>

            <!-- Modal footer -->
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-dark" data-dismiss="modal">{!! trans('admin.Close') !!}</button>
                <button type="submit" class="btn btn-outline-danger delete-company-{!! $id !!}" data-dismiss="modal">{!! trans('admin.Agree') !!}</button>
            </div>
...
{!! Form::open(['url' => aurl('companies/'.$id),'id'=>'form_delete_company_'.$id,'method'=>'delete']) !!}
...
<script>
    $(document).on('click','.delete-company-{!! $id !!}',function(){
        $('#form_delete_company_{!! $id !!}').submit();
    });
